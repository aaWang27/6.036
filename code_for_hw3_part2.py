...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test, iter):
    th, th0 = learner(data_train, labels_train, params={'T': iter})
    return score(data_test, labels_test, th, th0)/data_test.shape[1]

def xval_learning_alg(learner, data, labels, k, iter):
    _, n = data.shape
    idx = list(range(n))
    np.random.seed(0)
